const STORAGE_KEY = 'luis_territorial_quadras_v3';
...
function clonePadrao() {
  return JSON.parse(JSON.stringify(quadrasPadrao));
}

function carregarLocal() {
  try {
    const salvo = JSON.parse(localStorage.getItem(STORAGE_KEY));
    if (!Array.isArray(salvo)) return clonePadrao();

    return clonePadrao().map(padrao => {
      const existente = salvo.find(q => Number(q.id) === padrao.id);
      if (!existente) return padrao;
      const casas = Array.isArray(existente.casas) ? existente.casas : [];
      return {
        ...padrao,
        ...existente,
        casas: casas.map((casa, index) => ({
          ...casa,
          id: casa.id || `local-q${padrao.id}-${index + 1}`,
...
function mesclarImportados(quadrasBase) {
  const importados = obterImportados();
...
function salvar() {
  ...
  try {
    ...
    alert('O armazenamento local ficou cheio. As fotos importadas pelo GitHub não causam esse problema; evite cadastrar muitas fotos pelo seletor antigo do navegador.');
  }
  renderizar();
  atualizarTotais();
}
...
function atualizarTotais() {
  recalcular();
  document.getElementById('stat-concluidas').innerText = `${quadras.filter(q => q.status === 'Captação Realizada').length} de ${quadras.length}`;
    ...
    card.innerHTML = `
      ...
      <div class="space-y-2.5">
        ...
        <div class="bg-[#050f1f] border border-[#11233f] rounded-xl px-4 py-2.5 flex items-center justify-between">
          ...
          <span class="text-sm font-bold text-white">${q.imoveis}</span>
        </div>
        <div class="bg-[#050f1f] border border-[#11233f] rounded-xl px-4 py-2.5 flex items-center justify-between">
          <span class="text-xs text-slate-300 font-medium">Contatos Feitos</span>
          <span class="text-sm font-bold text-white">${q.contatos}</span>
        </div>
      </div>`;

...
let limitesQuadras = (() => {
  try { return JSON.parse(localStorage.getItem(LIMITES_QUADRAS_KEY) || '{}'); }
  catch { return {}; }
})();
...
function instalarFiltroGeografico() {
  const aguardar = () => {
    const modalAssistente = document.getElementById('modal-assistente-lotes');
    if (!modalAssistente || typeof buscarLotesDaRua !== 'function') {
      setTimeout(aguardar, 120);
      return;
    }
    if (document.getElementById('assist-bairro')) {
      conectarBotaoDefinirArea();
      return;
    }
    ...
    if (bloco) {
      ...
      rua.parentElement.insertAdjacentHTML('afterend', `
        ...
        <div class="flex items-end"><button id="btn-definir-area-quadra" type="button" class="w-full px-4 py-2 bg-indigo-700 hover:bg-indigo-600 text-white text-xs font-bold rounded-lg flex items-center justify-center gap-2"><i data-lucide="scan" class="w-4 h-4"></i> Definir área</button></div>`);
    ...
    if (aviso && !document.getElementById('assist-area-status')) {
    ...
    if (!document.getElementById('modal-area-quadra')) {
      document.body.insertAdjacentHTML('beforeend', `
        <div id="modal-area-quadra" class="fixed inset-0 bg-black/90 backdrop-blur-md z-[95] hidden items-center justify-center p-3 md:p-5">
          <div class="bg-[#081427] border border-[#1b355e] w-full max-w-5xl max-h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div class="p-5 border-b border-[#122543] flex items-center justify-between gap-3">
              ...
              <button id="btn-fechar-area-quadra" type="button" class="text-slate-400 hover:text-white p-2"><i data-lucide="x" class="w-6 h-6"></i></button>
            </div>
            <div class="p-4 flex flex-wrap gap-2 border-b border-[#122543] bg-[#050f1f]">
              <button id="btn-area-minha-localizacao" type="button" class="px-3 py-2 bg-emerald-700 hover:bg-emerald-600 text-white text-xs font-bold rounded-lg">Minha localização</button>
              <button id="btn-area-desfazer" type="button" class="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold rounded-lg">Desfazer ponto</button>
              <button id="btn-area-limpar" type="button" class="px-3 py-2 bg-rose-950 hover:bg-rose-900 text-rose-200 text-xs font-semibold rounded-lg">Limpar desenho</button>
              <button id="btn-area-salvar" type="button" class="ml-auto px-4 py-2 bg-indigo-700 hover:bg-indigo-600 text-white text-xs font-bold rounded-lg">Salvar área da quadra</button>
            </div>
            <div id="mapa-area-quadra" style="height:min(72vh,680px);min-height:430px;background:#07152a"></div>
          </div>
        </div>`);
    }
...
function conectarBotaoDefinirArea() {
  const btn = document.getElementById('btn-definir-area-quadra') || document.querySelector('button[onclick="abrirDefinicaoAreaQuadra()"]');
  if (!btn || btn.dataset.areaConectada === '1') return;
  btn.dataset.areaConectada = '1';
...
function conectarControlesModalArea() {
  const ligacoes = [
    ['btn-fechar-area-quadra', fecharDefinicaoAreaQuadra],
    ['btn-area-minha-localizacao', usarMinhaLocalizacaoArea],
    ['btn-area-desfazer', desfazerPontoArea],
    ['btn-area-limpar', limparAreaQuadra],
    ['btn-area-salvar', salvarAreaQuadra]
  ];
  ligacoes.forEach(([id, fn]) => {
    const el = document.getElementById(id);
    if (!el || el.dataset.conectado === '1') return;
...
function abrirDefinicaoAreaQuadra() {
  ...
  const modal = document.getElementById('modal-area-quadra');
  if (!modal) {
    alert('O mapa de definição de área não foi carregado. Atualize a página com Ctrl + F5 e tente novamente.');
    return;
  }
  ...
  pontosLimiteQuadra = Array.isArray(limitesQuadras[assistenteQuadraId])
    ? limitesQuadras[assistenteQuadraId].map(p => [Number(p[0]), Number(p[1])])
    : [];
  ...
  setTimeout(() => {
    try {
      ...
      if (!mapaLimiteQuadra) {
        mapaLimiteQuadra = L.map('mapa-area-quadra', { zoomControl: true }).setView([-22.9528, -43.0612], 17);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
          maxZoom: 20,
          attribution: '&copy; OpenStreetMap contributors'
        }).addTo(mapaLimiteQuadra);
      ...
      mapaLimiteQuadra.invalidateSize(true);
      renderizarDesenhoArea();
      if (pontosLimiteQuadra.length >= 3) {
        mapaLimiteQuadra.fitBounds(L.latLngBounds(pontosLimiteQuadra), { padding: [30, 30] });
      }
    } catch (erro) {
      console.error(erro);
      const alvo = document.getElementById('mapa-area-quadra');
      if (alvo) alvo.innerHTML = '<div style="padding:24px;color:#fecaca;font-size:13px">Não foi possível carregar o mapa. Atualize a página com Ctrl + F5 e tente novamente.</div>';
    }
  }, 120);
}
window.abrirDefinicaoAreaQuadra = abrirDefinicaoAreaQuadra;
...
window.fecharDefinicaoAreaQuadra = fecharDefinicaoAreaQuadra;

function renderizarDesenhoArea() {
  if (!camadaLimiteQuadra || typeof L === 'undefined') return;
  camadaLimiteQuadra.clearLayers();
  pontosLimiteQuadra.forEach((p, i) => {
    L.circleMarker(p, { radius: 6, color: '#818cf8', fillColor: '#6366f1', fillOpacity: 1 })
      .addTo(camadaLimiteQuadra)
      .bindTooltip(String(i + 1), { permanent: true, direction: 'top' });
  });
  if (pontosLimiteQuadra.length >= 2) {
    L.polyline(pontosLimiteQuadra, { color: '#818cf8', weight: 2, dashArray: '5,5' }).addTo(camadaLimiteQuadra);
  }
  if (pontosLimiteQuadra.length >= 3) {
    L.polygon(pontosLimiteQuadra, { color: '#6366f1', weight: 2, fillColor: '#4f46e5', fillOpacity: .15 }).addTo(camadaLimiteQuadra);
  }
}
...
window.desfazerPontoArea = desfazerPontoArea;

function limparAreaQuadra() {
  pontosLimiteQuadra = [];
  renderizarDesenhoArea();
}
window.limparAreaQuadra = limparAreaQuadra;

function usarMinhaLocalizacaoArea() {
  if (!navigator.geolocation) return alert('Este navegador não oferece geolocalização.');
  navigator.geolocation.getCurrentPosition(
    pos => {
      if (!mapaLimiteQuadra) return;
      mapaLimiteQuadra.setView([pos.coords.latitude, pos.coords.longitude], 18);
    },
    () => alert('Não foi possível obter sua localização.'),
    { enableHighAccuracy: true, timeout: 12000 }
  );
}
window.usarMinhaLocalizacaoArea = usarMinhaLocalizacaoArea;

function salvarAreaQuadra() {
  if (pontosLimiteQuadra.length < 3) return alert('Marque pelo menos 3 pontos ao redor da quadra.');
  limitesQuadras[assistenteQuadraId] = pontosLimiteQuadra.map(p => [Number(p[0]), Number(p[1])]);
...
window.salvarAreaQuadra = salvarAreaQuadra;

function pontoDentroPoligono(lat, lng, poligono) {
  let dentro = false;
  for (let i = 0, j = poligono.length - 1; i < poligono.length; j = i++) {
    const yi = Number(poligono[i][0]), xi = Number(poligono[i][1]);
    const yj = Number(poligono[j][0]), xj = Number(poligono[j][1]);
...
async function buscarLotesFiltradosPorArea() {
  ...
  if (!rua) {
    if (status) status.innerText = 'Digite o nome da rua para buscar.';
    return;
  }
  ...
  if (status) status.innerText = `Consultando somente ${rua} + ${bairro} + área desta quadra...`;

  try {
    const termoRua = normalizarTextoBusca(rua).replace(/'/g, "''");
    const termoBairro = normalizarTextoBusca(bairro).replace(/'/g, "''");
    const lats = poligono.map(p => Number(p[0]));
    const lngs = poligono.map(p => Number(p[1]));
    ...
    assistenteLotes = features.map((f, i) => {
      ...
      return {
        id: i,
        inscricao: a.tx_insct || '',
        rua: a.tx_logrado || rua,
        numero: a.tx_nroport || '',
        bairro: a.tx_bairro || bairro,
        latitude: c.lat,
        longitude: c.lng
      };
    ...
    if (status) {
      status.innerText = assistenteLotes.length
...
window.buscarLotesFiltradosPorArea = buscarLotesFiltradosPorArea;
